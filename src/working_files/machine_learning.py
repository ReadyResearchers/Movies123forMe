...
def m_learning():
    option = ['Opus', 'Netflix', 'Prime', 'Disney']
    dataset = st.selectbox("Select dataset: ", option)
    if dataset == 'Opus':
        opus = etl.clean_data()[0]

        # data processing
        x_val = st.selectbox("Select Value to Analyze: ", opus.columns[:])
        x = opus.drop([x_val], axis=1)
        y = opus[x_val]
    ...
    if dataset == 'Netflix':
        ...
        pd.get_dummies(netflix, dtype='uint8')
        # data processing
        x_val = st.selectbox("Select Value to Predict: ", netflix.columns[:])
        x = netflix.drop([x_val, 'title', 'director', 'cast', 'country', 'date_added', 'duration', 'description'], axis=1)
        y = netflix[x_val]
        netflix
    ...
    if dataset == 'Prime':
        ...
        # data processing
        x_val = st.selectbox("Select Value to Analyze: ", prime.columns[:])
        x = prime.drop([x_val], axis=1)
        y = prime[x_val]
        ...
        st.pyplot(plt.plot())
    if dataset == 'Disney':
        disney = etl.clean_data()[0]

        # data processing
        x_val = st.selectbox("Select Value to Analyze: ", disney.columns[:])
        x = disney.drop([x_val], axis=1)
        y = disney[x_val]
